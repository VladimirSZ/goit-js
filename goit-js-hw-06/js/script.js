...
// const calculateTotalBalance = users => users.reduce((totalSum, user) => totalSum + user.balance, 0);
...
// const getUsersWithFriend = (users, friendName) => users.filter(user => user.friends.indexOf(friendName) !== -1).map(user => user.name);
...
//       return allSkills;
//     }, [])
//     .filter((skill, idx, allSkills) => allSkills.indexOf(skill) === idx)
//     .sort();
// };
